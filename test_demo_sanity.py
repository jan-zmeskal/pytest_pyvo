...
@pytest.fixture(scope='module')
def json_file(json_dir, request):
...
def test_lowest(cp):
    assert cp.lowest().startswith('Holy See'), 'Holy smokes, Vatican is not the smallest!'

@pytest.mark.parametrize("test_input,expected", [
    ('Slovakia', '49012.0'),
    ('Czech Republic', '78866.0'),
    ('Germany', '357022.0')
])
def test_get(cp, test_input, expected):
    assert cp.get_value_for(test_input).split()[-1] == expected
